...
Or https://www.gov.uk/government/publications/standard-industrial-classification-of-economic-activities-sic
...
def parse_nace(nace_paths):
    ...
    def parse_single_file(nace_path, env=None):
        nace = {}
        with nace_path.open(encoding='utf-8') as f:
...
def parse_data_grep(nace, data_paths, dest_paths, categ_range):
    time_tag = get_time_tag()
    for categ, categ_range in nace.items():
        out_filename = f'{categ}__{time_tag}{data_paths[0].suffix}'
        out_paths = mapl(lambda p: p.joinpath(out_filename), dest_paths)

        escape_first_n_fields = f'(?:[^,]+,){{{nace_start_index},}}?.*?'
        ...
        try:
            ...
            with contextlib.ExitStack() as stack:
                ...
                out_files_cms = mapl(lambda p: p.open(
                    'w', encoding='utf-8'), out_paths)
                mapl(stack.enter_context, out_files_cms)
                out = MultiOut(**out_files_cms)

                base_command = [
                    'grep', '-E', f"{escape_first_n_fields}(?:{nace_range}).*?business\.data\.gov"]
                commands = [
                    [
                        *base_command,
                        str(data_path.relative_to(Path.cwd()))
            ...
            for out_path in out_paths:
                if not out_path.stat().st_size:
                    unlink_path(out_path)

        except BaseException as e:
            logger.error(f'Command failure: {repr(e)}')
            mapl(unlink_path, out_path)
...
def parse_data_python(nace, data_paths, dest_paths, suffix=None):
    ...
    for categ, categ_range in nace.items():
        ...
        log_file_creation(out_paths)
        mapl(lambda p: p.open('w', encoding='utf-8').close(), out_paths)

    for data_path in data_paths:
        with data_path.open(newline='', encoding='utf-8') as in_f:
                ...
                for categ, categ_range in nace.items():
                    ...
                    if any(q in s for q in categ_range for s in nace_vals):
                        with contextlib.ExitStack() as stack:
                            # context managers of opened files
                            logger.debug(
                                f'Adding "{datarow[debug_company_index]}" to "{categ}"')
                            out_files_cms = mapl(lambda p: p.open(
                                'a', newline='', encoding='utf-8'), out_paths_by_categ[categ])
                            mapl(stack.enter_context, out_files_cms)
