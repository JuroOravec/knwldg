...
from common import util, components
...
class SKMoJRegistrySpider(
    ABC,
    components.LimitedResultsResolver
    components.UrlModifier
):
    """
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            'sk.pipelines.CsvWriterPipeline': 400
        }
    }
    ...
    def __init__(self, sources=None, sources_delim=None, *args, **kwargs):
        ...
        self.sources_delim = sources_delim or " "
        self.logger.debug(
            f'Sources delimiter set to "{self.sources_delim}"')

        self.sources = filter(None, (sources or "").split(self.sources_delim))
        self.logger.debug('Sources: "{}"'.format('", "'.join(self.sources)))
    ...
    def is_result_limited(self, total):
        return total > self.max_entries_per_page
...
class SKMoJBusinessesByNameSpider(
    SKMoJRegistrySpider,
    components.LimitedResultsByQueryResolver,
    scrapy.Spider
):
    """
    Scrapes entries from the Business Registry of Slovak Ministry of Justice
    ...
    def __init__(self, sources=None, *args, **kwargs):
        ...
        self.sources = (sources or "").split()
        self.string_combinations.variants = {'c': ['ch']}
    ...
    def process_source_line(self, line):
        ...
        name = q_obj.get(self.query_param, "")
        page = int(q_obj.get("STR", 1))
        i = len(name) - 1

        for url in self.search(name=name, page=page):
            yield url
    ...
    def search(self, name="", page=1):
        names = self.string_combinations.generate(seed=name)
        for name in names:
